...
from .serializers import (
    ...
    ReportCreateSerializer, ReportRunSerializer
)
...
def run_load_test(request, pk):
    ...
    # 发送WebSocket消息的辅助函数
    def send_websocket_message(report_id, message, status=None, stats=None, progress=None):
        """发送WebSocket消息的辅助函数"""
    ...
    def run_test():
        try:
            ...
            report.success_rate = (total_requests - total_failures) / total_requests * 100 if total_requests > 0 else 0
...
def export_report_pdf(request, pk):
    ...
    summary_info = [
        ...
        ['成功请求数', str((report.total_requests or 0) - (report.total_failures or 0))],
        ['失败请求数', str(report.total_failures or 0)],
        ['成功率', f"{report.success_rate:.2f}%" if report.success_rate else "0.00%"],
        ['平均响应时间', f"{report.avg_response_time:.2f}ms" if report.avg_response_time else "N/A"],
        ['RPS', f"{report.rps:.2f}" if report.rps else "N/A"],
        ['P50响应时间', f"{report.p50_response_time:.2f}ms" if report.p50_response_time else "N/A"],
        ['P90响应时间', f"{report.p90_response_time:.2f}ms" if report.p90_response_time else "N/A"],
        ['P95响应时间', f"{report.p95_response_time:.2f}ms" if report.p95_response_time else "N/A"],
        ['P99响应时间', f"{report.p99_response_time:.2f}ms" if report.p99_response_time else "N/A"],
    ]
    ...
    if request_stats.exists():
        request_headers = ['请求名称', '方法', '请求数', '失败数', '平均响应时间(ms)', 'P95响应时间(ms)', 'P99响应时间(ms)']
        ...
        for stat in request_stats:
            request_data.append([
                ...
                f"{stat.avg_response_time:.2f}" if stat.avg_response_time else "N/A",
                f"{stat.p95_response_time:.2f}" if stat.p95_response_time else "N/A",
                f"{stat.p99_response_time:.2f}" if stat.p99_response_time else "N/A",
            ])
    ...
    response['Content-Disposition'] = f'attachment; filename="report_{report.id[:8]}_{report.name}.pdf"'
    return response
...
def export_report_excel(request, pk):
    ...
    if request_stats.exists():
        ...
        ws.append([])
        ws.append(["请求详细统计"])
        ws.append(["请求名称", "方法", "请求数", "失败数", "平均响应时间(ms)", "P95响应时间(ms)", "P99响应时间(ms)"])
        ...
        for stat in request_stats:
            ws.append([
                ...
                round(stat.avg_response_time, 2) if stat.avg_response_time else "N/A",
                round(stat.p95_response_time, 2) if stat.p95_response_time else "N/A",
                round(stat.p99_response_time, 2) if stat.p99_response_time else "N/A",
            ])
    ...
    response['Content-Disposition'] = f'attachment; filename="report_{report.id[:8]}_{report.name}.xlsx"'
    return response
