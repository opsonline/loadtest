...
'''
    ...
    def _generate_tasks(self, requests):
        ...
        for i, req in enumerate(requests):
            ...
            # 处理 headers
            headers_str = json.dumps(req.headers, ensure_ascii=False) if req.headers else "{}"
...
'''
    ...
    def run_load_test(self, report, locust_file):
        ...
        cmd = [
            ...
            '--headless',
            '-u', str(report.users),
            '-r', str(report.spawn_rate),
            '--run-time', f"{report.duration}s",
            '--csv', str(reports_dir / f'report_{report.id}'),
            '--json',
        ]
