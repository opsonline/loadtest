...
class ScenarioCopyView(generics.GenericAPIView):
    ...
    def post(self, request, pk):
        ...
        return Response(
            {"code": 0, "message": "复制成功", "data": {"id": str(new_scenario.id)}}
        )
...
def import_har(request):
    ...
    try:
        ...
        # 提取 host
        extracted_host = parser.extract_host(har_file)
        target_host = host_replacement if host_replacement else extracted_host

        # 重新读取文件内容（因为 extract_host 已经读取过了）
        har_file.seek(0)
        entries = parser.parse(har_file, resource_types.split(","))

        if not entries:
            return Response(
                {"code": 400, "message": "HAR 文件中没有找到可导入的请求"},
...
def scenario_stats(request, pk):
    ...
    stats = {
        ...
        "last_run": scenario.reports.filter(status="completed")
        .order_by("-created_at")
        .first(),
    }
    ...
    return Response({"code": 0, "message": "success", "data": stats})
