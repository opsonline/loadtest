...
class Scenario(models.Model):
    ...
    har_file_name = models.CharField(max_length=255, blank=True, null=True, verbose_name='HAR文件名')
...
class Request(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    scenario = models.ForeignKey(Scenario, on_delete=models.CASCADE, related_name='requests')
    ...
    body_type = models.CharField(max_length=10, choices=BODY_TYPES, default='none', verbose_name='请求体类型')
    ...
    # 数据源绑定
    datasource = models.ForeignKey('datasources.DataSource', on_delete=models.SET_NULL, 
                                   null=True, blank=True, related_name='requests')
    datasource_mapping = models.JSONField(default=dict, blank=True, verbose_name='数据源映射')
...
class HARImport(models.Model):
    """HAR 导入记录"""
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    scenario = models.ForeignKey(Scenario, on_delete=models.CASCADE, related_name='har_imports')
    file_name = models.CharField(max_length=255, verbose_name='文件名')
    file_path = models.CharField(max_length=500, verbose_name='文件路径')
    resource_types = models.JSONField(default=list, verbose_name='资源类型过滤')
    host_replacement = models.CharField(max_length=500, blank=True, null=True, verbose_name='Host替换')
