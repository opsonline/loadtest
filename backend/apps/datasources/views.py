...
class DataSourceDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        detail_serializer = DataSourceDetailSerializer(instance)
        return Response(
...
def upload_file(request):
    ...
    if "file" not in request.FILES:
        return Response(
            {"code": 400, "message": "请上传文件"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    if file_name.endswith(".csv"):
        ...
        return Response(
            {"code": 400, "message": "不支持的文件类型，请上传 CSV 或 JSON 文件"},
    ...
    try:
        ...
        return Response(
            {"code": 500, "message": f"文件解析失败: {str(e)}"},
...
def test_connection(request, pk):
    ...
    datasource = get_object_or_404(
        DataSource, pk=pk, created_by=request.user, is_active=True
    )
    ...
    try:
        ...
        if success:
            return Response({"code": 0, "message": "连接成功"})
        else:
            return Response(
                {"code": 400, "message": "连接失败"}, status=status.HTTP_400_BAD_REQUEST
            )

    except Exception as e:
        return Response(
            {"code": 500, "message": f"连接失败: {str(e)}"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
        ...
        return Response(
            {
                "code": 0,
                "message": "success",
                "data": {"preview": data, "total_count": total_count},
            }
        )

    except Exception as e:
        return Response(
            {"code": 500, "message": f"获取数据失败: {str(e)}"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def list_source_types(request):
    ...
    return Response({"code": 0, "message": "success", "data": types})
