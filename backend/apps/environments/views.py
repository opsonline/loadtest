...
from .serializers import (
    EnvironmentListSerializer, EnvironmentDetailSerializer,
    EnvironmentCreateUpdateSerializer, VariableSerializer
)
...
class EnvironmentListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        queryset = Environment.objects.filter(created_by=self.request.user, is_active=True)
    ...
    def perform_create(self, serializer):
        ...
        if serializer.validated_data.get('is_default'):
            Environment.objects.filter(created_by=self.request.user, is_default=True).update(is_default=False)
...
def set_default_environment(request, pk):
    ...
    # 取消其他默认环境
    Environment.objects.filter(created_by=request.user, is_default=True).update(is_default=False)
