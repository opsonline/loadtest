...
class TestSuiteDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        detail_serializer = TestSuiteDetailSerializer(instance)
        return Response(
...
class TestCaseListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return TestCase.objects.filter(
            suite_id=suite_id, suite__created_by=self.request.user, is_active=True
        )
    ...
    def perform_create(self, serializer):
        suite = get_object_or_404(
            TestSuite, pk=self.kwargs.get("suite_id"), created_by=self.request.user
        )
...
class AssertionDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self):
        return Assertion.objects.filter(
            test_case__suite__created_by=self.request.user, is_active=True
        )
...
def execute_test(request):
    ...
    if test_case_id:
        # 执行单个测试用例
        test_case = get_object_or_404(
            TestCase, pk=test_case_id, suite__created_by=request.user, is_active=True
        )
        result = execute_single_test(test_case)
        return Response({"code": 0, "message": "测试完成", "data": result})

    else:
        # 执行整个套件
        suite = get_object_or_404(
            TestSuite, pk=suite_id, created_by=request.user, is_active=True
        )
        ...
        return Response({"code": 0, "message": "测试套件执行完成", "data": results})
